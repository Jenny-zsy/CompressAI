...
def test_checkpoint(model, test_loader, args):
    with torch.no_grad():
        ...
        for i, data in enumerate(test_loader):
            ...
            print("img {} psnr:{:.6f} ms-ssim:{:.6f} ms-ssim-DB:{:.6f} sam:{:.4f} bpp:{:.6f} enc_time{:.4f} dec_time{:.4f}".format(
                name, PSNR, MS_SSIM, MS_SSIM_DB, SAM, bpp, enc_time, dec_time))
            #f.write("img{} psnr:{:.6f} ms-ssim:{:.6f} bpp:{:.6f}\n".format(i+1, PSNR, MS_SSIM, bpp.item()))
            

    print("Average psnr:{:.6f} ms-ssim:{:.6f} ms-ssim-DB:{:.6f} sam{:.4f} bpp:{:.6f} enc_time{:.4f} dec_time{:.4f}".format(
        sumPsnr / len(test_loader), sumMsssim / len(test_loader), sumMsssimDB / len(test_loader), sumSAM / len(test_loader), 
        sumBpp / len(test_loader), sumEnctime / len(test_loader), sumDectime / len(test_loader)))
    ...
    return {
        ...
        "bpp": sumBpp / len(test_loader),
        "enc_time": sumEnctime / len(test_loader), 
        "enc_time": sumDectime / len(test_loader),
    }
...
if __name__ == "__main__":
    ...
    checkpoint = torch.load(os.path.join(model_path, 'checkpoint_1000.pth'))
