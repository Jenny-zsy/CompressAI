...
class SymmetricalTransFormer(nn.Module):
    def __init__(self,
        ...
        self.end_conv = nn.Sequential(nn.Conv2d(embed_dim, embed_dim * patch_size ** 2, kernel_size=5, stride=1, padding=2),
                                      ...
                                      nn.Conv2d(embed_dim, 31, kernel_size=3, stride=1, padding=1),
                                      )
    ...
    def forward(self, x):
        ...
        #print("y_hat reshape: ", y_hat.shape)
        for i in range(self.num_layers):
            layer = self.syn_layers[i]
            y_hat, Wh, Ww = layer(y_hat, Wh, Ww)
            #print("dec %d y_hat: "%i, y_hat.shape)
