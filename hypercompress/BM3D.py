...
def BM3D_1st_step(_noisyImg):
    ...
    Basic_img[:, :] /= m_Wight[:, :]
    basic = numpy.matrix(Basic_img, dtype=float)
    basic.astype(numpy.float64)
...
def BM3D_2nd_step(_basicImg, _noisyImg):
    ...
    m_img[:, :] /= m_Wight[:, :]
    Final = numpy.matrix(m_img, dtype=float)
...
if __name__ == '__main__':
    ...
    # img_name = "/data3/zhaoshuyi/compressresults/cheng2020_CAVE_chN192_chM192_lambda0.01_beta0.1_bs16_lr0.0001/checkpoint1000_1.0N/1inputs.png"  # 图像的路径
    # img = cv2.imread(img_name, cv2.IMREAD_GRAYSCALE)    # 读入图像，cv2.IMREAD_GRAYSCALE:以灰度模式读入图像
    # print(img)
    data = sio.loadmat("/data3/zhaoshuyi/compressresults/cheng2020_CAVE_chN192_chM192_lambda0.01_beta0.1_bs16_lr0.0001/checkpoint1000_1.0N/egyptian_statue_ms.mat")
    data = data['inputs']
    img = data[:,:,0]
    ori = sio.loadmat("/data3/zhaoshuyi/Datasets/CAVE/hsi/test/egyptian_statue_ms.mat")['data']
    ori = ori[0]/1.932
    #print(img)

    # 记录程序运行时间
    e1 = time.time()  # cv2.getTickCount 函数返回从参考点到这个函数被执行的时钟数
    # if(img is not None):
    #     print("success")
    Basic_img = BM3D_1st_step(img)
    e2 = time.time()
    t = e2 - e1 # 计算函数执行时间
    print ("The Processing time of the First step is %f s" % t)
    print(np.uint8(Basic_img*255).clip(0, 255))
    Basic = Image.fromarray(np.uint8(Basic_img*255).clip(0, 255))
    #print(Basic)
    Basic.save("Basic3.jpg")
    #cv2.imwrite("Basic3.jpg", Basic_img)
    Metric = iv.spectra_metric(ori, Basic_img)
    psnr =  Metric.PSNR()
    ...
    e3 = time.time()
    t = e3 - e2
    print ("The Processing time of the Second step is %f s" % t)

    Final = Image.fromarray(np.uint8(Final_img*255).clip(0, 255))
    Final.save("Final3.jpg")
    #cv2.imwrite("Final3.jpg", Final_img)
    Metric = iv.spectra_metric(ori, Final_img)
    psnr =  Metric.PSNR()
    print ("The PSNR between the two img of the Second step is %f" % psnr)
    t = e3 - e1
    print ("The total Processing time is %f s" % t)
